ALTER TABLE public.bank_transactions ADD COLUMN IF NOT EXISTS is_forecast boolean NOT NULL DEFAULT false;
CREATE INDEX IF NOT EXISTS bank_transactions_forecast_idx ON public.bank_transactions (account_id, tipo, valor) WHERE is_forecast;
...
BEGIN
  ...
  SELECT id, data, descricao INTO _f FROM public.bank_transactions
   WHERE account_id = NEW.account_id AND is_forecast AND tipo = NEW.tipo AND valor = NEW.valor
     AND NEW.data BETWEEN data AND data + 3
...
DROP TRIGGER IF EXISTS bank_tx_replace_forecast ON public.bank_transactions;
CREATE TRIGGER bank_tx_replace_forecast AFTER INSERT ON public.bank_transactions
FOR EACH ROW EXECUTE FUNCTION public.bank_tx_replace_forecast();