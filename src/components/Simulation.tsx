          ...
          <div className="border-t border-border pt-4 space-y-2">
            ...
            {extras.map(ex => (
              <div key={ex.id} className="flex gap-2 items-center">
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full shrink-0 ${ex.tipo === 'entrada' ? 'bg-primary/10 text-primary' : 'bg-destructive/10 text-destructive'}`}>{ex.tipo === 'entrada' ? 'REC' : 'DES'}</span>
        ...
        <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
          <div className="glass-card rounded-xl p-6 space-y-3">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="border-t border-border pt-2 flex justify-between font-semibold"><span>Total Receitas</span><span className="text-primary">{formatCurrency(receitaSimulada)}</span></div>
              ...
              <div className="border-t border-border pt-2 flex justify-between text-lg font-bold font-display"><span>Resultado</span><span className={resultadoSimulado >= 0 ? 'text-primary' : 'text-destructive'}>{formatCurrency(resultadoSimulado)}</span></div>
            </div>
          </div>
          <div className={`glass-card rounded-xl p-6 ${metaValor > 0 ? 'glow-orange' : 'glow-green'}`}>
            <div className="flex items-center gap-2 mb-3"><Target className={`w-5 h-5 ${metaValor > 0 ? 'text-secondary' : 'text-primary'}`} /><h3 className="font-display font-semibold text-sm">Meta para não ficar negativo</h3></div>
            {metaValor > 0 ? (<div className="space-y-1 text-sm"><p>Valor necessário: <span className="font-semibold text-secondary">{formatCurrency(metaValor)}</span></p>{metaMatriculas > 0 && <p>Ou <span className="font-semibold text-secondary">{metaMatriculas}</span> matrículas adicionais</p>}</div>) : (<p className="text-sm text-primary font-medium">Resultado positivo! ✓</p>)}
          </div>
        </motion.div>
      </div>
    </div>
