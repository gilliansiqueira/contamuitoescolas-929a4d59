  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card rounded-xl p-5"><span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Resultado</span><p className={`text-2xl font-display font-bold ${resultado >= 0 ? 'text-primary' : 'text-destructive'}`}>{formatCurrency(resultado)}</p></motion.div>
      </div>
      {negativeDays.length > 0 && firstNeg && (<motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-xl p-4 border-destructive/30 bg-destructive/5"><div className="flex items-start gap-3"><AlertTriangle className="w-5 h-5 text-destructive mt-0.5 shrink-0" /><div><h4 className="font-display font-semibold text-sm text-destructive">Risco de Caixa — {scenarioLabel}</h4><p className="text-xs text-muted-foreground mt-1">Saldo negativo em <span className="font-semibold text-foreground">{firstNeg.data}</span> ({formatCurrency(firstNeg.saldo)}). {negativeDays.length} dia(s) com saldo negativo.</p></div></div></motion.div>)}
      {scenarioData.length > 0 && (<motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass-card rounded-xl p-6"><h3 className="text-base font-display font-semibold mb-4 text-foreground">Projeção de Saldo — {scenarioLabel}</h3><ResponsiveContainer width="100%" height={300}><AreaChart data={scenarioData}><defs><linearGradient id={`grad-${scenario}`} x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={scenarioColor} stopOpacity={0.25} /><stop offset="95%" stopColor={scenarioColor} stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="hsl(210, 15%, 88%)" /><XAxis dataKey="data" tick={{ fontSize: 11, fill: 'hsl(210, 10%, 45%)' }} /><YAxis tick={{ fontSize: 11, fill: 'hsl(210, 10%, 45%)' }} /><Tooltip contentStyle={{ background: 'hsl(0,0%,100%)', border: '1px solid hsl(210,15%,88%)', borderRadius: '8px', fontSize: '12px' }} formatter={(value: number) => formatCurrency(value)} /><ReferenceLine y={0} stroke="hsl(0, 72%, 50%)" strokeDasharray="4 4" strokeWidth={1.5} /><Legend /><Area type="monotone" dataKey="saldo" name={`Saldo (${scenarioLabel})`} stroke={scenarioColor} fill={`url(#grad-${scenario})`} strokeWidth={2} /></AreaChart></ResponsiveContainer></motion.div>)}
    </div>
  );
}
