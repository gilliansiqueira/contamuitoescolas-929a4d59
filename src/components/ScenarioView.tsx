...
export function ScenarioView({ schoolId, scenario, selectedMonth }: ScenarioViewProps) {
  ...
  // SSOT — prazo aplicado, ignorar removido, modelo aplicado
  const { entries: allEntries, saldoInicial } = useProjectedEntries(schoolId);
  ...
  const scenarioData = useMemo(() => {
    ...
    entries.forEach(e => {
      ...
      else byDate[data].saidas += Math.abs(impact);
    });
  ...
  }, [entries, allEntries, scenario, reductionPct, sales, saldoInicial]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass-card rounded-xl p-5"><span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Resultado</span><p className={`text-2xl font-display font-bold ${resultado >= 0 ? 'text-success' : 'text-destructive'}`}>{formatCurrency(resultado)}</p></motion.div>
      </div>
