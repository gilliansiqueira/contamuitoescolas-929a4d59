...
export function FinancialCalendar({ schoolId, selectedMonth }: FinancialCalendarProps) {
  ...
  const days: (number | null)[] = []; for (let i = 0; i < firstDayOfWeek; i++) days.push(null); for (let i = 1; i <= daysInMonth; i++) days.push(i);
    ...
    <div className="space-y-4">
      <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="glass-card rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          <button onClick={() => navigate(-1)} className="p-1.5 rounded-lg hover:bg-muted transition-colors"><ChevronLeft className="w-5 h-5 text-muted-foreground" /></button>
          <h3 className="font-display font-semibold text-foreground">{monthNames[month - 1]} {year}</h3>
          <button onClick={() => navigate(1)} className="p-1.5 rounded-lg hover:bg-muted transition-colors"><ChevronRight className="w-5 h-5 text-muted-foreground" /></button>
        </div>
        <div className="grid grid-cols-7 gap-1 mb-1">{dayNames.map(d => (<div key={d} className="text-center text-xs font-medium text-muted-foreground py-1">{d}</div>))}</div>
        <div className="grid grid-cols-7 gap-1">
          {days.map((day, i) => {
            if (day === null) return <div key={`empty-${i}`} />;
            const dateStr = `${viewMonth}-${String(day).padStart(2, '0')}`;
            const dd = dayData[dateStr]; const saldo = dd ? dd.entradas - dd.saidas : 0; const hasData = !!dd; const isNegative = hasData && saldo < 0; const isSelected = selectedDay === dateStr;
            return (<button key={day} onClick={() => hasData && setSelectedDay(isSelected ? null : dateStr)} className={`relative p-1 rounded-lg text-xs min-h-[56px] flex flex-col items-center justify-start transition-all ${isSelected ? 'ring-2 ring-primary bg-primary/10' : isNegative ? 'bg-destructive/10 hover:bg-destructive/20' : hasData ? 'bg-primary/5 hover:bg-primary/10' : 'hover:bg-muted/50'} ${hasData ? 'cursor-pointer' : 'cursor-default'}`}>
              <span className={`font-medium ${isNegative ? 'text-destructive' : 'text-foreground'}`}>{day}</span>
              {hasData && (<div className="mt-0.5 space-y-0.5 w-full text-center"><div className="text-[9px] text-primary font-medium leading-none">+{(dd.entradas / 1000).toFixed(0)}k</div><div className="text-[9px] text-destructive font-medium leading-none">-{(dd.saidas / 1000).toFixed(0)}k</div></div>)}
            </button>);
          })}
        </div>
      </motion.div>
      <AnimatePresence>
        {selectedDay && selectedDayData && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="glass-card rounded-xl overflow-hidden">
            <div className="px-5 py-3 flex items-center justify-between border-b border-border/50"><h4 className="font-display font-semibold text-sm text-foreground">{selectedDay.split('-').reverse().join('/')}</h4><button onClick={() => setSelectedDay(null)} className="p-1 rounded hover:bg-muted"><X className="w-4 h-4" /></button></div>
            <div className="grid grid-cols-3 gap-3 px-5 py-3 border-b border-border/30">
              <div className="text-center"><p className="text-xs text-muted-foreground">Entradas</p><p className="text-sm font-bold text-primary">{formatCurrency(selectedDayData.entradas)}</p></div>
              <div className="text-center"><p className="text-xs text-muted-foreground">Saídas</p><p className="text-sm font-bold text-destructive">{formatCurrency(selectedDayData.saidas)}</p></div>
